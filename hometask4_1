...
UNION  -- выводит с дубликатами

(SELECT *
FROM clients);


-- Проверьте, присутствует ли буква “А” в последовательности 'A', 'B', 'C', 'D'  IN
SELECT "A" IN ("A", "B", "C", "D") As Res; -- если выводит 1, то пристуствует (TRUE)/ если отсутсвует, то 0

-- Получите все логины из таблицы “users” , кроме "Mikle" IN
SELECT login
FROM users
WHERE "Mikle" NOT IN(login); --  или WHERE login NOT IN "Mikle"
